...
describe('linksnip', function(){
    ...
    it('msnbc article test', function(done){
        linksnip("http://www.nbcnews.com/technology/space-all-small-cheap-satellites-may-one-day-do-your-6C10488674", function(results){
            ...
            assert.equal(results.description, "SAN FRANCISCO &#8212; Someday, swarms of satellites the size of a tissue box will be snapping pictures, taking environmental readings and...")
            assert.equal(results.images.length, 1)
            assert.equal(results.images[0].height, 241)
            assert.equal(results.images[0].width, 362)
            assert.equal(results.images[0].format, 'JPEG')
            assert.equal(results.images[0].url, 'http://media4.s-nbcnews.com/j/streams/2013/July/130701/6C8110926-130530-151719-nanosat-1815.blocks_desktop_tease.JPG')
            done()
        })
    })
    ...
    it('msnb.com test', function(done){
        linksnip("http://msnb.com", function(results){
            assert.equal(results.url, "http://msnb.com/")
            assert.equal(results.title, "Lakeland Bank - Home")
            assert.equal(results.description, "Lakeland Bank -Community oriented bank located in Northern New Jersey. Provides financial products and services to local small businesses and consumers.")
            assert.equal(results.images.length, 3)
            assert.equal(results.images[0].height, 84)
            assert.equal(results.images[0].width, 136)
            assert.equal(results.images[0].format, 'JPEG')
            assert.equal(results.images[0].url, 'http://msnb.com/LakelandBank/media/lakelandbankmedia/FinancialServices_thumb.jpg')
